...
const linksURL = 'https://eluwa101.github.io/wdd230/links.json';

// Define an asynchronous function to fetch links data
async function getLinks() {
  try {
    ...
    console.log(data); // Test JSON result (remove this line once testing is done)
    displayLinks(data.weeks); // Call displayLinks function with data as argument
  } catch (error) {
    console.error('Error:', error);
  }
}

// Call the getLinks function to start fetching data
getLinks();
...
function displayLinks(weeks) {
  ...
  weeks.forEach((weekObj) => {
    ...
    weekLink.href = '#'; // Placeholder link
